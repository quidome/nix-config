    ...
    bar = {
      ...
      widgets = {
        ...
        in
          ...
          ++ [bluetoothWidget]
          ++ [volumeWidget]
          ++ (optional cfg.enableBrightnessWidget brightnessWidget)
          ++ (optional cfg.enableNetworkWidget networkWidget)
          ++ [
            {
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # Merge generated settings with user settings, forcing Nix-controlled keys
    home.activation.noctaliaSettings = lib.hm.dag.entryAfter ["writeBoundary"] ''
      if [ -f "${settingsFile}" ]; then
        ${pkgs.jq}/bin/jq -s '
          .[0] * .[1] * {
            appLauncher: { terminalCommand: .[0].appLauncher.terminalCommand }
          }
        ' "${settingsGenerated}" "${settingsFile}" > "${settingsFile}.tmp" \
          && mv "${settingsFile}.tmp" "${settingsFile}"
      else
        cp "${settingsGenerated}" "${settingsFile}"
      fi
    '';
